...
class TablaNotasPresupuesto extends tablaSQL.TablaSQL {
    ...
    // Tratamiento del campo Nota

    setNota(notax) {
        this.nota = notax;
        super.setString("NOTA", this.nota);
    }
    getNota() {
        this.nota = super.getString("NOTA");
        return this.nota;
    }
    addFirstNota(opcio,valo) {
        super.addFirstTermString("NOTA", opcio, valo);
    }
    addAndNota(opcio, valo) {
        super.addAndTermString("NOTA", opcio, valo);
    }
    addOrNota(opcio,valo) {
        super.addOrTermString("NOTA", opcio, valo);
    }
    ...
    setPrimaryIndex() {
        this.pstmtin.push("SELECT * FROM NOTAS_PPTO WHERE HOJA = $1 AND LINEA = $2");
        this.pstmtin.push("DELETE FROM NOTAS_PPTO WHERE HOJA = $1  AND LINEA = $2");
    }

    setCampos() {
        super.addCampo("HOJA", Number.class);
        super.addCampo("LINEA", Number.class);
        super.addCampo("NOTA", String.class);
    }

}
module.exports = {
    TablaNotasPresupuesto: TablaNotasPresupuesto
}