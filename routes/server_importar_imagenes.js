// --------------------------------------------------------------
// Servidor: Importar Imagenes Aplicacion
// --------------------------------------------------------------
...
var upload = multer({ dest: path.join(__dirname, '../img_cliente') });

/* GET home page. */
router.get('/importar_imagenes', async function (req, res, next) {
    ...
    fs.readdir(folder, (err, files) => {
        ...
        var content;
        fs.readFile('./views/importar_imagenes.pug', function read(err, data) {
            if (err) {
                throw err;
            }
            content = data;
...
router.post('/importar_imagenes', upload.single('get_fichero'), async function (req, res, next) {
