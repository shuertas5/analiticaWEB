// --------------------------------------------------------------
// Servidor: Impresion de la Analitica en Tramite
// --------------------------------------------------------------
...
/* GET home page. */
router.get('/imprimir_analitica_en_tramite', async function (req, res, next) {
    ...
    await tablaParametrosApp.getByPrimaryIndex().then(stat2 => {
        ...
        var content;
        fs.readFile('./views/imprimir_analitica_en_tramite.pug', async function read(err, data) {
            if (err) {
                throw err;
            }
            content = data;
            res.send(pug.render(content, { titulo_boton, color_boton, mes_actual, mes_en_calculo_format, formato_listado, hoja_inicial, hoja_final, campos: campos }));
        });
    });
});

router.post('/imprimir_analitica_en_tramite', async function (req, res, next) {
                        ...
                        while (stat == true) {
                            ...
                            await tablaCifras.getByPrimaryIndex(hoja, linea).then(async stati => {
                                ...
                                if (stati == true) {
                                    ...
                                    valor_ante = tablaCifras.getValorAnnoAnteriorEnTramite(mes_actual);
                                }
                                else {
                                    valor_mes = 0.0;
                                    valor_acumulado = 0.0;
                                ...
                                await tablaCifras.getByPrimaryIndex(ref_hoja, ref_linea).then(stati => {
                                    ...
                                    if (stati == true) {
                                        ...
                                        valor_ante_ref = tablaCifras.getValorAnnoAnteriorEnTramite(mes_actual);
                                    }
                                    else {
                                        valor_mes_ref = 0.0;
                                ...
                                if (tipo_acumulado == tabla_lineas.TablaLineas.ACUMULADO_DATO_EXTRA) {
                                    ...
                                    await tablaCifrasExtras.getByPrimaryIndex(hoja, linea).then(async stati => {
                                        ...
                                        if (stati == true) {
                                            ...
                                            valor_ante = tablaCifrasExtras.getValorMesAnnoAnteEnTramite(mes_actual);
                                        }
                                        else {
                                            valor_acumulado = 0.0;
                                            valor_ante = 0.0;
                                    ...
                                    await tablaCifrasExtras.getByPrimaryIndex(ref_hoja, ref_linea).then(async stati => {

                                        if (stati == true) {
                                            valor_acumulado_ref = tablaCifrasExtras.getValorMesEnTramite();
                                            valor_ante_ref = tablaCifrasExtras.getValorMesAnnoAnteEnTramite(mes_actual);
                                        }
                                        else {
                                            valor_acumulado_ref = 0.0;
                                            valor_ante_ref = 0.0;
                                ...
                                if (tipo_acumulado == tabla_lineas.TablaLineas.ACUMULADO_MEDIA_12_MESES) {
                                    ...
                                    await tablaCifras.getByPrimaryIndex(hoja, linea).then(async stati => {
                                        ...
                                        if (stati == true) {
                                            ...
                                            await tablaHistoricoCifras.getByPrimaryIndex(anno_en_curso - 2, hoja, linea).then(async stati => {

                                                if (stati == true) {
                                                    for (var i = mes_consu + 1; i <= 12; i++) {
                                                        valoresmeses[i] = tablaHistoricoCifras.getValorMes(i);
                                                    }
                                                    valor_ante = 0.0;
                                                    for (var i = 1; i <= 12; i++) {
                                                        valor_ante += valoresmeses[i];
                                    ...
                                    await tablaCifras.getByPrimaryIndex(ref_hoja, ref_linea).then(async stati => {
                                        ...
                                        if (stati == true) {
                                            ...
                                            await tablaHistoricoCifras.getByPrimaryIndex(anno_en_curso - 2, ref_hoja, ref_linea).then(async stati => {
                            ...
                            if (imp.hayespacio(1) == true && invisible == false) {
                                ...
                                if (blanca == true && caracterblanco != ' ' && caracterblanco != '\0') {
                                    ...
                                    if (color == true) setColorNumero(imp, valor_mes, de_totales, estadistica, invisible);
                                    ...
                                    if (intensa == true) imp.setNegrita(true);
                                    if (color == true) setColorNumero(imp, valor_ante, de_totales, estadistica, invisible);
                                    if (con_decimales == true) {
                                    ...
                                    imp.print(" ");
                                    if (intensa == true) imp.setNegrita(true);
                                    if (color == true) setColorNumero(imp, valor_acumulado, de_totales, estadistica, invisible);
                                    if (con_decimales == true) {
                                    ...
                                    if (intensa == true) imp.setNegrita(true);
                                    if (color == true) setColorNumero(imp, valor_presupuesto, de_totales, estadistica, invisible);
                                    if (con_decimales == true) {
...
function cabecera(imp, tituloempresa, hojaana, titulohoja, externa, mes, anno) {
    ...
    imp.print("  " + rutinas.acoplarserie(tituloempresa, 40) + "  ");
    imp.printre(' ', 4);
    imp.print("[");
    imp.print(formato.form("##", hojaana, "0"));
    imp.print("] ");
    imp.print(rutinas.acoplarserie(titulohoja, 30));
    ...
    imp.printre(' ', 3);
    imp.print("          MES                   C O N C E P T O           ACUM. AÑO ANTERIOR       ACUMULADO ACTUAL           PRESUPUESTO       ");
    imp.linea();
...
function setColorNumero(imp, valor, detotales, estadistica, lineainvisible) {
