// --------------------------------------------------------------
// Servidor: Impresion del Historico de la Analitica
// --------------------------------------------------------------
...
/* GET home page. */
router.get('/imprimir_historico_analitica', async function (req, res, next) {

    const current_url = new URL(req.protocol + "://" + req.get('host') + req.originalUrl);
    const search_params = current_url.searchParams;

    var anno = search_params.get('anno');
    ...
    await tablaParametrosApp.getByPrimaryIndex().then(stat2 => {
        ...
        var content;
        fs.readFile('./views/imprimir_historico_analitica.pug', async function read(err, data) {
            if (err) {
                throw err;
            }
            content = data;
            res.send(pug.render(content, { titulo_boton, color_boton, mes_actual, meses, anno_actual, annos, annos_str, formato_listado, hoja_inicial, hoja_final, campos: campos }));
        });

    });
});

router.post('/imprimir_historico_analitica', async function (req, res, next) {
    ...
    var anno_consu = rutinas.leer_lista_campos(vector_texto, 'ANNO');
        ...
        await tablaHojas.open().then(async reto1 => {
            ...
            while (stat0 == true) {
                ...
                if (hoja >= hoja_inicial && hoja <= hoja_final && hoja_invisible == false) {
                    ...
                    cabecera(imp, nombre_empresa, hoja, titulohoja, externa, mes_consu, anno_consu);
                    ...
                    await tablaLineas.open().then(async ret => {
                        ...
                        while (stat == true) {
                            ...
                            await tablaHistoricoCifras.getByPrimaryIndex(anno_consu - 1, hoja, linea).then(async stati => {
                                ...
                                if (stati == true) {
                                    valor_ante = tablaHistoricoCifras.getValorAcumulado(mes_consu);
                                }
                                else {
                                    valor_ante = 0.0;
                                }
                                ...
                                await tablaHistoricoCifras.getByPrimaryIndex(anno_consu - 1, ref_hoja, ref_linea).then(stati => {
                                    ...
                                    if (stati == true) {
                                        valor_ante_ref = tablaHistoricoCifras.getValorAcumulado(mes_consu);
                                    }
                                    else {
                                        valor_ante_ref = 0.0;
                                    }
                            ...
                            if (sumable == false) {
                                ...
                                if (tipo_acumulado == tabla_lineas.TablaLineas.ACUMULADO_DATO_EXTRA) {
                                    ...
                                    await tablaHistoricoCifrasExtras.getByPrimaryIndex(anno_consu - 1, hoja, linea).then(async stati => {
                                        ...
                                        if (stati == true) {
                                            valor_ante = tablaHistoricoCifrasExtras.getValorMes(mes_consu);
                                        }
                                        else {
                                            valor_ante = 0.0;
                                        }
                                    ...
                                    await tablaHistoricoCifrasExtras.getByPrimaryIndex(anno_consu - 1, ref_hoja, ref_linea).then(stati => {
                                        ...
                                        if (stati == true) {
                                            valor_ante_ref = tablaHistoricoCifrasExtras.getValorMes(mes_consu);
                                        }
                                        else {
                                            valor_ante_ref = 0.0;
                                        }
                                ...
                                if (tipo_acumulado == tabla_lineas.TablaLineas.ACUMULADO_MEDIA_12_MESES) {
                                    ...
                                    await tablaHistoricoCifras.getByPrimaryIndex(anno_consu - 2, ref_hoja, ref_linea).then(async stati => {
                                        ...
                                        if (stati == true) {
                                            ...
                                            valor_ante_ref = valor_acumulado_ref / 12;
                                        }
                                        else {
                                            valor_ante_ref = 0.0;
                                        }
                            ...
                            await tablaHistoricoPresupuesto.getByPrimaryIndex(anno_consu, hoja, linea).then(async stati => {
                                ...
                                if (stati == true) {
                                    valor_presupuesto = tablaHistoricoPresupuesto.getValorAcumulado(mes_consu);
                                }
                                else {
                                ...
                                await tablaHistoricoPresupuesto.getByPrimaryIndex(anno_consu, ref_hoja, ref_linea).then(stati => {
                                    ...
                                    if (stati == true) {
                                        valor_presupuesto_ref = tablaHistoricoPresupuesto.getValorAcumulado(mes_consu);
                                    }
                                    else {
