// --------------------------------------------------------------
// Servidor: Impresion del Historico del Presupuesto
// --------------------------------------------------------------
...
/* GET home page. */
router.get('/imprimir_historico_presupuesto', async function (req, res, next) {
    ...
    await tablaParametrosApp.getByPrimaryIndex().then(stat2 => {
        ...
        if (anno == null) anno = anno_en_curso - 1;
        ...
        var content;
        fs.readFile('./views/imprimir_historico_presupuesto.pug', async function read(err, data) {
            if (err) {
                throw err;
            }
            content = data;
...
router.post('/imprimir_historico_presupuesto', async function (req, res, next) {
        ...
        await tablaHojas.open().then(async reto1 => {
            ...
            while (stat0 == true) {
                ...
                if (hoja >= hoja_inicial && hoja <= hoja_final && hoja_invisible == false) {
                    ...
                    cabecera(imp, nombre_empresa, hoja, titulohoja, externa, anno_consu, mes_consu);
...
function cabecera(imp, nombre_empresa, hojaana, titulohoja, externa, annoppto, mes) {
